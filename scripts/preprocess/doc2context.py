"""
Converts the output of readalign_to_docs.py into a file suitable for training a concatenation based model, i.e. with context sentences separated by a <context> token.
Length of context is defined by the parameters described below.
Outputs truecased and not truecased versions of the corpus.

Usage: python3 docs2context.py fn srcLn tgtLng context_len_src context_len_tgt [context_len_tgttgt]
where fn is the input file name
srcLng, tgtLng are the source and target languages
context_len_src: number of previous target sentences on the source side
context_len_tgt: number of previous target sentences on the source side
context_len_tgttgt: number of previous target sentences on the target side
