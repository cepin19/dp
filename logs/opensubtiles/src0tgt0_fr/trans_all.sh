...
n=$(bc -l <<< $n/10);
echo $n
base_model=model/model.src0tgt0.newvocab.iter630000.npz

vocab=corp/vocab.encz.opensub.new.yml
$marian/marian-decoder  -d $dev  -n $n -b 6 -v $vocab $vocab  -m  $base_model --mini-batch 16 -i corp/opensub.en-fr.docs.dev.en.bpe  > out_test
echo $n  >> base 
echo >>  base
echo "real" >>base
echo >> base
...
cat out_test_post | python3 -m sacrebleu corp/opensub.en-fr.docs.dev.fr  >> base
echo >> base
rm out_test
rm out_test_post
