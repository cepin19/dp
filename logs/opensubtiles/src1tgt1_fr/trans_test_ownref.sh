...
dir=`dirname "$0"`
vocab=$dir/corp/vocab.encz.opensub.yml

echo $model >> out_test_"$sys_name"_desc
echo $n >> out_test_"$sys_name"_desc
$marian/marian-decoder --relative-paths  -d $dev  -n $n -b 6 -v $vocab $vocab  --maxi-batch 100 --maxi-batch-sort src -m  $model --mini-batch 32 -i $dir/corp/opensub.en-fr.docs.test.en.bpe.owntgt > out_test_$sys_name
