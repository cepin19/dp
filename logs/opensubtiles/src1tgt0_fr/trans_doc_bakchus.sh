...
marian=/home/large/data/models/marian/marian-doc/doc-marian3/build/
for n in {6..6}
do
n=$(bc -l <<< $n/10);
echo $n
concat_model=model/model.src1tgt0.doc.new.spider.newgate.iter200000.npz
concat_model=model/model.src1tgt0.doc.new.bakchus.newest.npz
...
$marian/marian-decoder --dim-vocabs 30000 30000 30000 --type transformer-context --context-gate -m model/model.src1tgt0.doc.new.bakchus.newest.npz --ignore-model-config -d $dev  -n $n -b 6 --maxi-batch 100 -v $vocab $vocab $vocab  -m  $concat_model --mini-batch 64 --maxi-batch-sort src -i corp/opensub.en-fr.docs.dev.en.bpe.src_prev  corp/opensub.en-fr.docs.dev.en.bpe.src  > out_test
