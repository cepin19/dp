...
def eval_with_paired_bootstrap(gold, sys1, sys2,
                               num_samples=10000,num_cores=1, sample_ratio=0.5,
                               eval_type='acc'):
    ...
    #    sys1_scores.append(sys1_score)
     #   sys2_scores.append(sys2_score)
        return sys1_score,sys2_score,wins
    #print (num_cores)
    # Print win stats
    sys1_scores, sys2_scores,wins = zip(*Parallel(n_jobs=num_cores)(delayed(count_sample)() for _ in range(num_samples)))
    wins=[sum(x) for x in zip(*wins)]
    sys1_scores=list(sys1_scores)
    sys2_scores=list(sys2_scores)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', help='Number of cores to use', type=int, default=1)
    ...
    with open(args.sys2, 'r') as f:
        sys2 = f.readlines()
    eval_with_paired_bootstrap(gold, sys1, sys2, eval_type=args.eval_type, num_samples=args.num_samples, num_cores=args.c)
